...
def log_code_execution(code_execution_function):
    ...
    if code_execution_function.__name__ not in ("submit_python_job", "submit_scala_job"):
        raise ValueError("this should be only used to log submit_python_job and submit_scala_job now")
...
class ScalaJobHelper:
    def __init__(self, parsed_model: Dict, credential: Credentials) -> None:
        raise NotImplementedError("ScalaJobHelper is not implemented yet")

    def submit(self, compiled_code: str) -> Any:
        raise NotImplementedError("ScalaJobHelper submit function is not implemented yet")
...
class BaseAdapter(metaclass=AdapterMeta):
    ...
    def default_scala_submission_method(self) -> str:
        raise NotImplementedError("default_scala_submission_method is not specified")

    @log_code_execution
    def submit_scala_job(self, parsed_model: dict, compiled_code: str) -> AdapterResponse:
        submission_method = parsed_model["config"].get(
            "submission_method", self.default_python_submission_method
        )
        if submission_method not in self.scala_submission_helpers:
            raise NotImplementedError(
                "Submission method {} is not supported for current adapter".format(
                    submission_method
                )
            )
        job_helper = self.scala_submission_helpers[submission_method](
            parsed_model, self.connections.profile.credentials
        )
        submission_result = job_helper.submit(compiled_code)
        # process submission result to generate adapter response
